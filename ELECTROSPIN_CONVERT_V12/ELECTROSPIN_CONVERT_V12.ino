...
//set I/O
  ...
  attachInterrupt(digitalPinToInterrupt(Y_LIM), y_axis_limit_triggered, HIGH);
  attachInterrupt(digitalPinToInterrupt(X_LIM), x_axis_limit_triggered, HIGH);
  ...
  String command = Serial.readString();
   ...
   if (command.substring(0,command.indexOf(":")) == "YS") { //YS = Y_START (start position of y axis)
    ...
    if (command.substring(0,command.indexOf(":")) == "XP") { //YS = Y_START (start position of y axis)
      x_pos = command.substring(command.indexOf(":") + 1).toFloat()*X_STEPS_PER_MM;
...
void update_spindle(float spindle_sps){
    ...
    while(spindle_sps > current_spindle_sps){// accelerate to target speed
        ...
        delayMicroseconds(current_spindle_sps/8);//this is accel rate
    }
...
void home_axes(){
  ...
  Serial.println("waiting for limit");
  while(!Y_LIMIT);
...
void y_axis_limit_triggered(){
  ...
  if(current_y_dir){ //limit switch only active when y stage is moving towards it
...
Serial.println(y_pos_in_steps);
  ...
  if(y_pos_in_steps>y_axis_position){
    ...
    Serial.print("Y AXIS position before motion:  ");
    Serial.println(y_axis_position);
